(function () {
  ...
  if (cfg.pageDescription) {
    ...
    if (descEl) descEl.setAttribute("content", cfg.pageDescription);
  }
  ...
  /* ---------- monogram (date + groom/bride initials) ---------- */
  (function buildMonogram() {
    var datesEl = document.getElementById("monogram-dates");
    var coupleEl = document.getElementById("monogram-couple");
    if (!datesEl || !coupleEl) return;

    var parts = (cfg.eventDateShort || "").split(".");
    if (parts.length === 3) {
      var day = parts[0];
      var month = parts[1];
      var year = parts[2].slice(-2);
      datesEl.innerHTML =
        "<span>" + day + "</span>" +
    ...
    var names = (cfg.coupleNamesLatin || "").split("&").map(function (s) { return s.trim(); }).filter(Boolean);
    coupleEl.innerHTML = "";
    names.forEach(function (name, i) {
      if (i > 0) {
        var divider = document.createElement("div");
        divider.className = "monogram-divider-h";
        coupleEl.appendChild(divider);
      }
      var half = document.createElement("div");
      half.className = "monogram-half";
      half.innerHTML =
        '<span class="monogram-letter">' + name.charAt(0).toUpperCase() + "</span>" +
        '<span class="monogram-label">' + name.toUpperCase() + "</span>";
      coupleEl.appendChild(half);
    });
  })();
  ...
  (function buildCalendar() {
    ...
    var html = '<div class="calendar-month">' + (monthNames[month] || "") + " " + year + "</div>";
  ...
  /* ---------- typewriter-style line-by-line text reveal ---------- */
  document.querySelectorAll(".type-reveal").forEach(function (block) {
  ...
  /* ---------- reveal on scroll ---------- */
  var revealTargets = document.querySelectorAll(".reveal, .type-reveal, .photo-frame");
